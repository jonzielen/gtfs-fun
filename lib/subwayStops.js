  ...
  getTrainData(train) {
    var data = train.reduce(function(prev, curr, index, array) {
      if (curr.hasOwnProperty('trip_update') &&
          ...
          curr.trip_update.stop_time_update.map(function(stop, i) {
            stop.trip_id = curr.trip_update.trip.trip_id;
            stop.route_id = curr.trip_update.trip.route_id;
            prev.push(stop);
          });
      }
    ...
    var finalStops = stops.reduce(function(prev, curr, index, array) {
      ...
      data.map(function(elem, i) {
        if (curr.stop_id === elem.stop_id) {
          curr.train_lines.push(elem.route_id);
          train.push(elem);
        }
      });
      ...
      if (train.length > 0) {
        curr.train_lines = [...new Set(curr.train_lines)];
        prev.push({
          stop: curr
        });
      }
